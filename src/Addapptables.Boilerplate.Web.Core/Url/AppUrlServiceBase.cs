...
namespace Addapptables.Boilerplate.Url
{
    public abstract class AppUrlServiceBase : IAppUrlService, ITransientDependency
    {
        ...
        {
            var resetLink = WebUrlService.GetSiteRootAddress(tenancyName).EnsureEndsWith('/') + PasswordResetRoute + "/{userId}/{resetCode}";
        ...
        public string GetTenantUrl(int? tenantId)
        {
            return GetTenantUrl(GetTenancyName(tenantId));
        }

        public string GetTenantUrl(string tenancyName)
        {
            var resetLink = WebUrlService.GetSiteRootAddress(tenancyName).EnsureEndsWith('/') + "account/login";
